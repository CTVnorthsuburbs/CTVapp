...
public struct Vods: Decodable
{
    ...
    public init?(json: JSON)
    {
        ...
        guard let fileName: String = "fileName" <~~ json
            ...
            {
                print("no vod fileName here in JSON")
                return nil
            }
...
public struct Shows: Decodable
{
    ...
    public init?(json: JSON)
    {
        ...
        if(comments != nil) {self.comments = comments!}
...
public struct YoutubeThumbnail: Decodable
{
    ...
    public init?(json: JSON) {self.defaultThumbnail = "standard" <~~ json}
}

public struct YoutubeDefaultThumbnail: Decodable
{
    let url: String?
    public init?(json: JSON) {self.url = "url" <~~ json}
}

public struct YoutubeResource: Decodable
{
    let videoId: String?
    public init?(json: JSON) {self.videoId = "videoId" <~~ json}
}
