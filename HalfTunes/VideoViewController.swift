...
class VideoViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    func hidePlayer() {self.thumbnailButton.isHidden = true}
    func showPlayer() {self.thumbnailButton.isHidden = false}
    func setCategory(category: Category) {self.currentCategory = category}
    ...
    override func viewDidAppear(_ animated: Bool)
    {
        ...
        var show_the_player = false
        appDelegate.shouldRotate = false// or false to disable rotation
        if(videoLoaded == false) {show_the_player = true}
        if(video?.getIsEvent() == true)
        {
            if (video?.eventDate?.checkIfDateTimeIsNow())! {show_the_player = true}
            else {show_the_player = false}
        }
        if(show_the_player==false){hidePlayer()}
        else{showPlayer()}
    }
        ...
        DispatchQueue.global(qos: .background).async
            {
                if(video.fileName != nil  && self.currentCategory?.videoType != VideoType.youtube)
                {
                    if(video.fileName != 1)
                    {
                        ...
                        if(video.thumbnail == nil) {video.generateThumbnail()}
                    }
                }
                ...
                DispatchQueue.main.async(){LoadingOverlay.shared.hideOverlayView()}
        }
    }
    ...
    func loadEventDescription(video: Video)
    {
        ...
        if (date?.checkIfDateTimeIsTomorrow())!  {self.childView.dateLabel.text = "Tomorrow"}
        else if (date?.checkIfDateTimeIsToday())! {self.childView.dateLabel.text = "Today"}
        else {self.childView.dateLabel.text = date?.convertDateToString()}
        self.navigationItem.title = video.title
        self.childView.titleLabel.text   = video.title
    ...
    override func viewDidLoad()
    {
        ...
        if(self.currentCategory != nil) {self.childView.setCategory(category: self.currentCategory!)}
        else
        {
        ...
        if(self.loadVideos() != nil) {myVideos = self.loadVideos()!}
        if (self.hasSavedVideo()) {self.toggleAddButton()}
        
        self.thumbnailButton.isHidden = false
    }
    
    func setDefaultSession(defaultSession: inout Foundation.URLSession) {self.defaultSession = defaultSession}
    func setDataTask(dataTask: inout URLSessionDataTask) {self.dataTask = dataTask}
    func setDownloadsSession(downloadsSession: inout Foundation.URLSession) {self.downloadsSession = downloadsSession}
    ...
    @objc func setProgressBar()
    {
        ...
        if( tempDownload?.progress !=  nil ) { childView.progressView.setProgress(tempDownload!.progress, animated: true)}
        else
        {
    ...
    override func viewDidDisappear(_ animated: Bool){timer?.invalidate()}
    
    func loadVideos() -> [Video]? {return NSKeyedUnarchiver.unarchiveObject(withFile: Video.ArchiveURL.path) as? [Video]}
    
    @IBAction func cancel(_ sender: UIBarButtonItem)
    {
        // Depending on style of presentation (modal or push presentation), this view controller needs to be dismissed in two different ways.
        let isPresentingInAddVideoMode = presentingViewController is UINavigationController
        if isPresentingInAddVideoMode {dismiss(animated: true, completion: nil)}
    ...
    func addVideo(_ sender: AnyObject)
    {
        ...
        {
            if(!childView.addVideoButton.isSelected) {saveVideos()}
        }
        
        if(childView.addVideoButton.titleLabel?.text == "Save in Library"){downloadTapped()}
        if(childView.addVideoButton.titleLabel?.text == "Pause"){pauseDownload(video!)}
        if(childView.addVideoButton.titleLabel?.text == "Resume"){resumeDownload(video!)}
    }
    ...
    func hasSavedVideo() -> Bool
    {
        for vid in myVideos
        {
            if (vid.title == video?.title) {return true}
        }
        return false
    }
    ...
    func saveVideos()
    {
        ...
        if (isSuccessfulSave) {toggleAddButton()}
        else { print("Failed to save videos...")}
    }
    ...
    //IF the video is an upcoming event you can not bookmnark it. This is what this function does.
    func showAddButton()
    {
        if(video?.getIsEvent() == true){childView.addVideoButton.isHidden = true}
        else{childView.addVideoButton.isHidden = false}
    }
    ...
    func toggleAddButton()
    {
        ...
        if(video != nil)
        {
            ...
            else
            {
                if(hasSavedVideo())
                {
                    if(currentCategory?.videoType != VideoType.youtube &&  video?.getIsEvent() == false ) {childView.addVideoButton.setTitle("Save in Library", for: UIControlState.selected)}
                    else {childView.addVideoButton.setTitle("Bookmarked", for: UIControlState.selected)}
                    childView.addVideoButton.isSelected = true
                }
                else
                {
    ...
    func downloadTapped()
    {
        ...
        childView.progressView.isHidden = !showDownloadControls
        childView.cancelButton.isHidden = !showDownloadControls
        childView.addVideoButton.isHidden = !showDownloadControls
    }
    
    func cancelTapped(_ sender: AnyObject) {cancelDownload(video!)}
    
    @IBAction func pauseTapped(_ sender: AnyObject) {pauseDownload(video!)}
    ...
    {print("resuming download in myvideosviewcontroller")
        ...
        download.isDownloading = true
      //  childView.addVideoButton.setTitle("Pause", for: UIControlState.selected)
        childView.addVideoButton.isHidden = true
    }
}
