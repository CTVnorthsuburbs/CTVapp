...
class VideoSearch : UIViewController, UITableViewDelegate, UISearchBarDelegate
{
    ...
    func getCategories() -> [Category] {return categories}
    ...
    func search(_ savedSearchID: Int)-> [Video]
    {
        ...
        if(results != nil)
        {
            ...
            if (results!.count > arrayLength)
            {
                ...
                if (splitResults != nil)
                {
                    ...
                    for url in searchURLs {let _ = getSearchResults(defaultSession: session, url: url, isIDSearchURL: true)}
                }
            }
            else //if array is smaller than maximum, just process it
            {
                let searchIdURL =  convertIdArrayToSearchURL(results!)
                let _ = getSearchResults(defaultSession: session, url: searchIdURL!, isIDSearchURL: true)
            }
        }
        ...
        for id in originalResults!
        {
            ...
            {
                if(result.id == id) {sortedSearchResults.append(result)}
            }
        }
    ...
    */
            ...
            else if let httpResponse = response as? HTTPURLResponse
            {
                if httpResponse.statusCode == 200
                {
                    ...
                    do
                    {
                        ...
                        //  print(json)
                        guard let results = YoutubeVideo(json: json) else {return}
                        //  print(results)
                        //  var thumbnail = result
                        
                        for result in results.items!
                        {
                            guard let snippet = YoutubeItems(json: result as! JSON)
                                else
                                {
                                    print("no itmes")
                                    return
                            ...
                            if(videoSnippet?.thumbnail?.defaultThumbnail?.url != nil) {thumbnail = (videoSnippet?.thumbnail?.defaultThumbnail?.url)!}
                            else {thumbnail = nil}
                            ...
                            if(videoSnippet?.resourceId?.videoId != nil) {videoId  = (videoSnippet?.resourceId?.videoId)!}
    ...
    func searchForSingle(_ savedSearchID: Int)-> [Video]
    {
        ...
        let _ = getSearchResults(defaultSession: session, url: searchURL!, isIDSearchURL: true)
    ...
    func trimVideos(videoArray: [Video], numberToReturn: Int) -> [Video]
    {
        ...
        {
            if (count > 0) {reducedResults.append(result)}
            count = count - 1
        }
    ...
    fileprivate func getSearchResults( defaultSession: URLSession, url: URL, isIDSearchURL: Bool) -> [Int]?
    {
        ...
        dataTask = defaultSession.dataTask(with: url,  completionHandler:
        {
            ...
            else if let httpResponse = response as? HTTPURLResponse
            {
                if httpResponse.statusCode == 200
                {
                    if (isIDSearchURL == true) {let _ = self.updateSearchResults(data)}
                    else {results = self.getSavedSearchResults(data)!}
                }
                else
                {
                    //  self.getSearchResults(defaultSession, url: url, isIDSearchURL: isIDSearchURL)
                    print("!!!Results from video search not found")
                    let _ = self.getSearchResults(defaultSession: defaultSession, url: url, isIDSearchURL: isIDSearchURL)
                }
            }
            semaphore.signal()
        })
        ...
        dataTask = URLSession.shared.dataTask(with: url, completionHandler:
        {
            ...
            else if let httpResponse = response as? HTTPURLResponse
            {
                if httpResponse.statusCode == 200
                {
                    ...
                    guard let results = Thumbnail(json: json) else {return}
                    guard let result = results.thumbnail else {return}
    ...
    fileprivate func getSavedSearchResults(_ data: Data?) -> [Int]?
    {
        ...
        guard let VideosResult = AllVideos(json: json)
            else {return nil}
        
        guard let results = VideosResult.results
            else {return nil}
        
        guard let result = results["results"]
            else {return nil}
        
        let finalResult = result as! [Int]
        return finalResult
    }//func getSavedSearchResults
    ...
    fileprivate func updateSearchResults(_ data: Data?)-> Bool
    {
        ...
        guard let VideosResult = VideosResult(json: json)
            else {return false}
        
        guard VideosResult.show != nil else {return false}
        ...
        {
            if(show.showThumbnail.count != 0) {fileName = show.showThumbnail[2]}
            else {fileName = nil}
            
            let date = convertStringToDate(dateString: show.date)
            if(VideosResult.show?.count != VideosResult.vod?.count) {print("video results do not match: VideoSearch updateSearchResults()")}
...
extension Array
{
    ...
    {
        if self.count <= distance {return [self]}
        else
        {
