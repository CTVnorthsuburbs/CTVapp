...
class MainTableViewController: UITableViewController
{
    ...
    {
        if(newCategory.categoryTitle == featuredCategory.categoryTitle){self.parentCategory = featuredCategory}
        else{self.parentCategory = newCategory}
        category = self.parentCategory
        self.setSlider()
    }//setCategory
    ...
    func loadSearch()
    {
        ...
        if(searchResults.count == 0) {searchResults = search.getRecent()}
    ...
    override func viewDidLoad()
    {
        ...
        DispatchQueue.global(qos: .background).async
            {
                if(categoriesVideos.count == 0) {self.generateCategories()}
                self.loadSearch()
            }
    ...
    func generateCategories()
    {
        for video in categories
        {
            ...
            /*
             ...
             {
             if( vid.first?.hasThumbnailUrl())!{search.getThumbnail(url: (vid.first?.thumbnailUrl)!)}
             else
             {
            ...
            default:
                ...
                if (vid.first?.fileName != nil){/*//    search.getThumbnail(id: (vid.first?.fileName)!)*/categoriesVideos.append(vid.first!)}
                else{if(vid.first != nil) {/*//categoriesVideos.append(vid.first!)*/}}
            }//switch (video.categoryTitle)
        }//for video in categories
    ...
    func categoryPressed() {self.performSegue(withIdentifier: "categoryPressed", sender: self)}
    ...
    func changeSize(height: Int)
    {
      ...
      //  if(vc != nil) {vc?.resetSlidePosition()}
    }
    ...
    {
        if (segue.identifier == "embedSegue") {self.embeddedViewController = segue.destination as? HorizontalTableViewController}
        if (segue.identifier == "slideShow") {vc = segue.destination as? SlideShowViewController}
    }//func prepare
