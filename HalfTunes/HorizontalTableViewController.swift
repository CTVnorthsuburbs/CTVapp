...
class HorizontalTableViewController: UITableViewController
{
    ...
    {
        print("memory warning recevied and caches cleared")
    ...
    {
        if(category.categoryTitle == featuredCategory.categoryTitle && currentCategory?.categoryTitle == featuredCategory.categoryTitle) {saveFeaturedVideos()}
        self.updateTable()
    ...
    public func updateTable()
    {
        if(self.currentCategory?.categoryTitle != category.categoryTitle )
        {
            ...
            else
            {
                ...
                if(category.sections.count == 0) {category.createListing()}
                self.currentCategory = category
                if(self.videos.count == 0)
                {
                    DispatchQueue.main.async{LoadingOverlay.shared.showOverlay(view: self.parent?.view)}
                    DispatchQueue.global(qos: .background).async
                    {
                        var index = 0
                    ...
                    self.changeTableSize()
                    if(category.categoryTitle == featuredCategory.categoryTitle && self.currentCategory?.categoryTitle == featuredCategory.categoryTitle) {self.saveFeaturedVideos()}
                }
            }
        ...
        DispatchQueue.main.async
        {
            ...
            while (index < category.sections.count)
            {
                ...
                if(category.sections[index].searchID != nil)
                {
                    ...
                    {
                        if(category.sections[index].getDisplayCount() == nil) {self.videos.append(self.search.getYouTubeVideos(playlist: category.sections[index].sectionPlaylist!)!)}
                        else
                        {
                            let vids = self.search.getYouTubeVideos(playlist: category.sections[index].sectionPlaylist!)
                            let trimmedVids = self.search.trimVideos(videoArray: vids!, numberToReturn: category.sections[index].getDisplayCount()!)
    ...
    override func numberOfSections(in tableView: UITableView) -> Int
    {
        ...
        else {return category.sections.count}
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> HorizontalTableViewCell
    {
        ...
        if (section.sectionType == SectionType.upcomingEventList)
        {
            ...
            cell!.sectionLabel.text = section.sectionTitle
            if(section.displayCount == nil){section.displayCount = 15}
            return cell!
        }
...
extension HorizontalTableViewController: UICollectionViewDelegate, UICollectionViewDataSource
{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int
    {
        if(category.sections[safe: collectionView.tag]?.sectionType == SectionType.videoList || category.sections[safe: collectionView.tag]?.sectionType == SectionType.upcomingEventList)
        {
            /*
             ...
             //  let  count =  videos[collectionView.tag].count
             */
            if let count = videos[safe: collectionView.tag]?.count {return count}
            self.refreshTable()
            return 0
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if(category.sections[collectionView.tag].sectionType == SectionType.buttonWithTitle || category.sections[collectionView.tag].sectionType == SectionType.buttonNoTitle)
        {
            ...
            {
                if(category.sections[collectionView.tag].buttons[indexPath.row]?.imageOverlay != nil) {cells.textOverlay.text = category.sections[collectionView.tag].buttons[indexPath.row]?.imageOverlay}
                else {cells.textOverlay.text = nil}
                var thumbnail: UIImage?
        ...
        if(category.sections[collectionView.tag].sectionType == SectionType.upcomingEventList )
        {
            ...
            else  if (videos[indexPath.item].eventDate?.checkIfDateTimeIsTomorrow())! {cells.dateLabel.text = "Tomorrow"}
            else {cells.dateLabel.text = videos[indexPath.item].eventDate?.convertEventDateToString()}
        }
        
        if(category.sections[safe: collectionView.tag]?.sectionType == SectionType.videoList   )
        {
            var videos = [Video]()
            if(category.videoType == VideoType.youtube) {videos =  search.getYouTubeVideos(playlist: category.sections[collectionView.tag].sectionPlaylist!)!}
            else if(category.videoType == VideoType.cablecast)
            {
                if (listOfVideos.keys.contains(category.sections[collectionView.tag].searchID!)) { videos = listOfVideos[category.sections[collectionView.tag].searchID!]!}
                else
                {
            ...
            {
                if( videos[indexPath.item].hasThumbnailUrl()) { cells.thumbnail.image = self.search.getThumbnail(url: (videos[indexPath.item].thumbnailUrl)!)}
                else
                {
    ...
    func setCategory(newCategory: Category)
    {
        ...
        while (index < category.sections.count)
        {
            if(category.sections[index].searchID != nil)
            {
                if(category.sections[index].getDisplayCount() == nil) {videos.append(search.search(category.sections[index].searchID!))}
                else
                {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        if(category.sections[collectionView.tag].sectionType == SectionType.buttonNoTitle ||  category.sections[collectionView.tag].sectionType == SectionType.buttonWithTitle ||  category.sections[collectionView.tag].sectionType == SectionType.squareButtonWithTitle)
        {
            ...
            if(button?.type == ButtonType.webPage)
            {
                /*  //this opens the external web page from a button in a web view within the app  it was disabled because the web view does not support JWPlayer 6 which many videos are
                 let vc = UIStoryboard(name:"Main", bundle:nil).instantiateViewController(withIdentifier: "webView") as! WebViewController---------deleted the web view controller
                 vc.setTitle(title: (button?.title)!)
                 vc.setPage(url: (button?.webURL)!)
                 self.navigationController?.pushViewController(vc, animated:true)
                 */
                //This opens the webpage in safari
                if let url =  button?.webURL {UIApplication.shared.open(url, options: [:], completionHandler: nil)}
               // if let url =  button?.webURL {UIApplication.shared.openURL(url)}
            }//if(button?.type == ButtonType.webPage)
        ...
        }// if(category.sections[collectionView.tag].sectionType == Sect...........
    }//func collectionView
}
