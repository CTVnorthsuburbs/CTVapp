...
class MyVideosSearchViewController: MyVideosViewController
{
    ...
    {print("thumnailTapped in MyVideosSearchViewController")
        if let indexPath = tableView.indexPath(for: cell)
        {
            var video = filtered[(indexPath as NSIndexPath).row]
            if(searchActive){video = filtered[(indexPath as NSIndexPath).row]}
            else {video = myVideos[(indexPath as NSIndexPath).row]}
            playVideo(video)
        }
    }
    ...
    @IBAction override func unwindToVideoList(_ sender: UIStoryboardSegue)
    {
        ...
        {
            if let selectedIndexPath = tableView.indexPathForSelectedRow {tableView.reloadRows(at: [selectedIndexPath], with: .none)}
            else
            {
                // Add a new video.
    ...
    override func videoIndexForDownloadTask(_ downloadTask: URLSessionDownloadTask) -> Int?
    {
        ...
        {
            for (index, video) in filtered.enumerated() {if url == video.sourceUrl! {return index}}
        }
        return nil
    }
}
