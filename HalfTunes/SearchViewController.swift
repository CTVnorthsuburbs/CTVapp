...
class SearchViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        if( retrievedData != nil) {savedResults = (NSKeyedUnarchiver.unarchiveObject(with: retrievedData!))! as! [Video]}
        
        if(savedResults.count != 0)
        {
            //set to != to use saved results when available, otherwise use the video search, == to always search (useful for testing).
            ...
            if(loadVideos() != nil) {otherVideos = loadVideos()!}
            for vid in otherVideos {searchResults.append(vid)}
    ...
    func updateSearchResults(_ searchResults: [Video]) {for item in searchResults {self.data.append(item.title!)}}
    ...
    override func viewDidAppear(_ animated: Bool) {indexChanged(segmentedControl)}
    func setMyVideoView()
    {
    ...
    @IBAction func indexChanged(_ sender: UISegmentedControl)
    {
        switch segmentedControl.selectedSegmentIndex
        {
            ...
            case 0:
                ...
                if(searchText!.isEmpty == false) {self.searchBar(searchBar, textDidChange:searchText!)}
                else {self.searchBar(searchBar, textDidChange:"")}
                
                if ((searchBar.text ) != nil) {allVideosResults.isHidden = false}
                else {allVideosResults.isHidden = true}
            ...
            case 1:
                ...
                if ((searchBar.text ) != nil) {myVideosResults.isHidden = false}
                else {myVideosResults.isHidden = true}
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String)
    {
        ...
        if (filtered.count > 0)
        {
            //this limits the results of the autocomplete for all Videos Search
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        else {cell.textLabel?.text = data[(indexPath as NSIndexPath).row]}
